...
j<-1
k<-1
Treatments<-c("CONTROL","BIOTIC","CHEMICAL")
...
for( i in 1:length(StudyPaths)){
  ...
  METAFILES<- list.files(path =StudyPaths[i], pattern = "META" )
  
  if(length(METAFILES) ==0){next}
  ...
  for (j in 1:length(Studyfiles)) {
    ...
    dir.create(paste("../Output_Tables/",StudyName,sep=""), showWarnings = FALSE)
    dir.create(paste("../Output_Plots/",StudyName,sep=""), showWarnings = FALSE)
    ...
    Metadata <-
      read.csv(paste(StudyPaths[i],"/",METAFILES[j],sep = ""),
               header = T,
               row.names = 1,
               check.names = F)
      ...
      #filter samples based on treatment 
      
      SAMPLES <-  ALLSAMPLES[which(Metadata$TREATMENT == Treatments[k]),]
    ...
    MantelOutput<-data.frame(Dataset=0,Treatment=0,Comparisons=0,MantelR=0, SimPval=0, HypothesisTest=0, Std.obs=0, Expectation=0, Variance=0)
    
    #mantel correlations
    ENZYCHEM.mantel <-
      mantel.randtest(ENZYMES.DIST, CHEMISTRY.DIST, nrepet = 9999)
    
    MantelOutput[1,]<-c(StudyName,Treatments[k],"EC",ENZYCHEM.mantel$obs,ENZYCHEM.mantel$pvalue ,ENZYCHEM.mantel$alter,ENZYCHEM.mantel$expvar[1],ENZYCHEM.mantel$expvar[2],ENZYCHEM.mantel$expvar[3])
    
    
    pdf(paste(currentPlotDirectory,"/",Treatments[k],"_Mantel_Chemisty_Enzyme.pdf", sep="")) # writing a PDF to file
    ...
    MantelOutput[2,]<-c(StudyName,Treatments[k],"CS",Chemistry.mantel$obs,Chemistry.mantel$pvalue ,Chemistry.mantel$alter,Chemistry.mantel$expvar[1],Chemistry.mantel$expvar[2],Chemistry.mantel$expvar[3])
    
    pdf(paste(currentPlotDirectory,"/",Treatments[k],"_Mantel_Chemistry_Samples.pdf", sep="")) # writing a PDF to file
    ...
    MantelOutput[3,]<-c(StudyName,Treatments[k],"ES",ENZYMES.mantel$obs,ENZYMES.mantel$pvalue ,ENZYMES.mantel$alter,ENZYMES.mantel$expvar[1],ENZYMES.mantel$expvar[2],ENZYMES.mantel$expvar[3])
    
    pdf(paste(currentPlotDirectory,"/",Treatments[k],"_Mantel_Enzyme_Samples.pdf", sep="")) # writing a PDF to file
    ...
    Meandistances<-data.frame(Study=StudyName,Treatment=Treatments[k], ChemistryDist=mean(CHEMISTRY.pairwise[, 3]),Chemistry.SD=sd(CHEMISTRY.pairwise[, 3]),Chemistry.SE=(sd(CHEMISTRY.pairwise[, 3]) / sqrt(length(CHEMISTRY.pairwise[, 3]))),
    ...
    Varpartition$TREATMENT<- Treatments[k]
    ...
    pdf(paste(currentPlotDirectory,"/",Treatments[k],"_VariancePartitioning.pdf", sep="")) # writing a PDF to file
    plot(
      varpartfullmodel,
      digits = 2,
      cutoff = 0.01,
...
write.csv(GlobalMantel,paste("../Output_Tables/Global","/Treatments_Mantel.csv",sep=""))
write.csv(GlobalCorrelation,paste("../Output_Tables/Global","/Treatments_Correlation.csv",sep=""))
...
write.csv(GlobalVariantionpartitioning,paste("../Output_Tables/Global/","Treatments_VariancePartition.csv",sep=""))

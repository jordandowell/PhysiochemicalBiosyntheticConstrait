...
MOLS <- load.molecules("Global_Compound_by_Enzyme_Table.sdf")
allDescs <- eval.desc(MOLS, get.desc.names())




#chemminerR list if you want to visualize any compound
sdfset <- read.SDFset("Global_Compound_by_Enzyme_Table.sdf")
#generate3D coordinates and add this to the sdf file
sdfset3d <- generate3DCoords(MOLS)
...
Chemistry.dend <- as.dendrogram(hclust(vegdist(CHEMISTRY, method = "gower"), method = "average"))
ENZYMES.dend <- (as.dendrogram(hclust(
  vegdist(ENZYMES,
          method = "gower"), method = "average"
)))
#CREATE COLOR LIST
...
#full data set Mantel
mantel(vegdist(CHEMISTRY, method = "gower"),
       vegdist(ENZYMES,
               method = "gower"))
...
labels <- Chemistry.dend %>% set("labels_to_char") %>% labels
...
#plot tangle gram
dend.chemenzy <- dendlist(Chemistry.dend,
                          ENZYMES.dend)
